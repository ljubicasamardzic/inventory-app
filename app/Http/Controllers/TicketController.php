...
class TicketController extends Controller
{
    ...
    {
        // check that the processing of the ticket has not yet started
        if ($ticket->status_id != Ticket::UNPROCESSED) {
            alert()->error('The ticket is being processed and cannot be updated any more!', 'Oops..');
        } else {
            if ($request->ticket_type == Ticket::NEW_EQUIPMENT && $request->ticket_request_type == Ticket::EQUIPMENT_REQUEST) {
                ...
                if ($ticket->update($request->all())) {
                    alert()->success('You have successfully updated your request1!', 'Success!');
                } else {
                    alert()->error('Something went wrong!', 'Oops..');
                }
                ...
                if ($ticket->update($request->all())) {
                    alert()->success('You have successfully updated your request2!', 'Success!');
                } else {
                    alert()->error('Something went wrong!', 'Oops..');
                } 
                ...
                if ($ticket->update($request->all())) {
                    alert()->success('You have successfully updated your request3!', 'Success!');
                } else {
                    alert()->error('Something went wrong!', 'Oops..');
                }
            }
    ...
    public function update_4(TicketRequest $request) {
        ...
        if ($ticket->isNewEquipmentRequest()) {
            ...
            if ($ticket->equipment_id != null && $ticket->isHRApproved() || !$ticket->isOfficerApproved() && $ticket->isHRApproved() || $ticket->isWaitingForEquipment() && $ticket->isHRApproved()) {
                ...
                if (!$new_doc || !$create_doc_item || !$update1) {
                    DB::rollBack();
                    alert()->error('Something went wrong!', 'Oops..');
                } else {
                    DB::commit();
    ...
    public function update_officer_decision($id, TicketRequest $request) {
        ...
        $this->authorize('update_officer_decision', $ticket);
         ...
         // check that the processing of the ticket has not yet started
        if ($ticket->HR_approval != Ticket::PENDING) {
            alert()->error('The HR has already responded to the request which is why you cannot update it any more!', 'Oops..');
        } else {     
            ...
            if ($ticket->isNewEquipmentRequest()) {
                if ($ticket->officer_approval == Ticket::APPROVED && $request->officer_approval == Ticket::APPROVED) {
                    ...
                    if ($ticket->equipment_id == null && $request->equipment_id != null) {
                        ...
                        if ($new_reservation && $update_ticket) {
                            DB::commit();
                            alert()->success('You have successfully updated the ticket!', 'Success!');
                        } else {
                            DB::rollBack();
    ...
    public function update_HR_decision($id, TicketRequest $request) {
        ...
        if ($ticket->date_finished != null) {
            DB::rollBack();
            alert()->error('The ticket has been marked as finished and cannot be updated any more!', 'Oops..');
        } else { 
            ...
            if ($ticket->HR_approval == Ticket::APPROVED && $request->HR_approval == Ticket::APPROVED) {
                ...
                if ($ticket->reservation != null) {
                    $delete_res = $ticket->reservation->delete();
                    if (!$delete_res) {
                        DB::rollBack();
                        alert()->error('Something went wrong!', 'Oops..');
                    }
                ...
                $update = $ticket->update($request->all());
                $update_status = $ticket->update(['status_id' => Ticket::IN_PROGRESS]);
